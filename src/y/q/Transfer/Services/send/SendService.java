...
import y.q.Transfer.Broadcast.WifiStateBroadcastReciver;
...
public class SendService extends Service
{
    ...
    public class SendAction extends Binder
    {
        ...
        public void connectionSSID(final String ssid, final List<Uri> tranList)
        {
            ...
            {
                @Override
                public void onWifiStateChanged(NetworkInfo info)
                {
                    if(info.getExtraInfo().equals("\""+ssid+"\""))
                    {
                        //的确已经连接到了指定的WIFI上，开始传输，否则由客户端再次请求连接到这个wifi上
                    }
                    else
                    {
                        connectionSSID(ssid, tranList);
                    }
                }
            });
        }
